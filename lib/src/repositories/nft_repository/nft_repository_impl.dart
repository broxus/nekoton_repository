...
mixin NftRepositoryImpl implements NftRepository {
  ...
  @override
  Stream<NftTransferEvent> get transferEventStream =>
      _transferEventSubject.stream;

  @override
  void subscribeToWalletNftTransfers(TonWallet wallet) {
  ...
  @override
  Future<NftCollection?> tryGetCollection({
    ...
    return currentTransport.transport.use(() async {
      ...
      try {
        return getCollection(nftItem?.nft.collection ?? address);
      } catch (e) {
    ...
    return currentTransport.transport.use(() async {
      final body = await encodeInternalInput(
        ...
        input: NftTransferInput(
          ...
          sendGasTo: sendGasTo,
          callbacks: {
            owner.toRaw(): NftCallbackPayload(
              value: BigInt.parse('100000000'),
              payload: '',
            ),
            recipient.toRaw(): NftCallbackPayload(
              value: BigInt.parse('100000000'),
              payload: '',
            ),
          },
  ...
  Future<InternalMessage> prepareNftTokenTransfer({
    ...
    return currentTransport.transport.use(() async {
      final (payload, _) = await packIntoCell(
        abiVersion: '2.2',
  ...
  Future<AccountsList> _getNftIndexes({
    ...
    final (boc, _) = await packIntoCell(
    ...
    final (_, hash) = await setCodeSalt(
      code: _indexCode,
      salt: boc,
    );
  ...
  Future<void> _handleTransactions({
    ...
    if (batchInfo.batchType == TransactionsBatchType.oldTransactions) return;
